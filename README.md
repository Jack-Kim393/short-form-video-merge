...
이 애플리케이션은 `linux/amd64` (일반적인 Intel/AMD CPU) 및 `linux/arm64` (Apple Silicon 등) 아키텍처를 모두 지원하는 멀티 아키텍처 Docker 이미지를 제공합니다. Docker가 설치되어 있다면 가장 간편하게 실행할 수 있습니다.

1.  **Docker 이미지 다운로드**:
    ```bash
    docker pull jack0399/video-merger:1.0
    ```

2.  **Docker 컨테이너 실행**:
    ```bash
    docker run --rm -p 7860:7860 jack0399/video-merger:1.1
    ```
    * `--rm`: 컨테이너가 중지될 때 자동으로 삭제되도록 설정합니다.
    * `-p 7860:7860`: 로컬 컴퓨터의 7860번 포트와 컨테이너의 7860번 포트를 연결합니다.

3.  **애플리케이션 접속**: 웹 브라우저에서 `http://localhost:7860` 주소로 접속합니다.
